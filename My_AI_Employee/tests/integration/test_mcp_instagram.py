#!/usr/bin/env python3
"""
Test Instagram Web MCP - FastMCP Implementation
Tests the Instagram browser automation MCP server.
"""
...
from instagram_web_mcp import mcp, InstagramBrowser


async def test_instagram_mcp():
    """Test Instagram Web MCP."""
    print("=" * 70)
    print("INSTAGRAM WEB MCP TEST (FastMCP)")
    print("=" * 70)
    print()
    ...
    try:
        async with InstagramBrowser() as browser:
            ...
            if logged_in:
                print(f"   ✅ Logged in to Instagram")
            else:
            ...
            print()
            print("Instagram Web MCP is ready to use!")
            print("Add to Claude Desktop with:")
            print("  claude mcp add instagram-web-mcp -s local \\")
            print("    -e VAULT_ROOT=AI_Employee_Vault \\")
            print("    -- uv run --directory /mnt/d/hackathon-0/My_AI_Employee \\")
            print("    python /mnt/d/hackathon-0/My_AI_Employee/mcp_servers/instagram_web_mcp.py")
            print()
...
if __name__ == "__main__":
    print()
    success = asyncio.run(test_instagram_mcp())
